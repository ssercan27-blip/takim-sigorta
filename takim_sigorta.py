...
def load_data_safe():
    try:
        ...
        # SÜTUNLARI TEMİZLE (Boşlukları sil, küçük harf yap)
        df.columns = [str(c).strip().lower().replace(" ", "_").replace("/", "_") for c in df.columns]
        # 'arsiv' verisini standardize et
...
if choice == "📝 Yeni Poliçe":
    ...
    if not df.empty:
        # FİLTRE: Boşlukları ve tip hatalarını temizleyerek sadece FALSE olanları al
    ...
    st.write(f"Sistemdeki tüm ham veriler (Google Sheets):")
    st.dataframe(df)
